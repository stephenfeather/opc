...
async function main() {
  ...
  if (queryType === "literal") {
    ...
    const context2 = `Search routing: literal query "${pattern}". For richer context (call graph, docstrings), also try: tldr search "${pattern}" ${projectDir} \u2014 or Read the target file (tldr-read-enforcer will enrich).`;
    const output2 = {
      hookSpecificOutput: {
  ...
  if (queryType === "structural") {
    ...
    const context2 = `Search routing: structural query "${pattern}". For precise AST matching, consider: mcp__ast-grep__find_code with pattern "${astPattern}". For richer context (call graph, docstrings): tldr search "${target || pattern}" ${projectDir}`;
    const output2 = {
      hookSpecificOutput: {
        hookEventName: "PreToolUse",
        additionalContext: context2
