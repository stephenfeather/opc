...
import { selectPattern, validateComposition, } from '../shared/pattern-selector.js';
import { gate3Composition, gate3CompositionChain, CompositionInvalidError, } from '../shared/composition-gate.js';
...
describe('Pattern Selector Functions', () => {
    ...
    describe('validateComposition', () => {
        ...
        it('validates 3-pattern chain', () => {
            const result = validateComposition(['pipeline', 'aggregator', 'pipeline'], 'handoff');
            expect(result.valid).toBe(true);
        });
    });
});
describe('Composition Gate', () => {
    ...
    describe('gate3CompositionChain', () => {
        it('validates 3-pattern chain', () => {
            const result = gate3CompositionChain(['pipeline', 'aggregator', 'pipeline'], 'handoff');
            expect(result.valid).toBe(true);
        });
...
describe('Python Bridge Error Paths', () => {
    ...
    describe('callValidateComposition error handling (via validateComposition)', () => {
        it('returns errors for Python script failures on invalid input', () => {
            ...
            const result = validateComposition(['__nonexistent__', '__also_nonexistent__'], 'handoff');
        ...
        it('handles error propagation through pairwise validation', () => {
            ...
            const result = validateComposition(['pipeline', '__invalid_pattern__'], 'handoff');
            expect(result.valid).toBe(false);
            // Error should mention the invalid pattern
            expect(result.errors.some((e) => e.toLowerCase().includes('unknown') || e.toLowerCase().includes('error'))).toBe(true);
        });
    });
...
describe('Edge Cases', () => {
    ...
    describe('Longer pattern chains', () => {
        it('validates 4-pattern chain', () => {
            const result = gate3CompositionChain(['pipeline', 'aggregator', 'pipeline', 'aggregator'], 'handoff');
            expect(result.valid).toBe(true);
        });
        it('fails early in chain when first pair invalid', () => {
            // First pair (jury, blackboard) should fail with iso scope
            expect(() => {
                gate3CompositionChain(['jury', 'blackboard', 'pipeline', 'aggregator'], 'iso');
            }).toThrow(CompositionInvalidError);
        });
...
describe('Integration Smoke Test', () => {
    ...
    it.skipIf(!process.env.RUN_INTEGRATION)('real Python validation works', () => {
        const result = validateComposition(['pipeline', 'aggregator'], 'handoff');
        expect(result.valid).toBe(true);
    });
