...
if echo "$CMD" | grep -qiE '(curl|wget).*(-X\s*POST|--data|--upload|-d\s)'; then
    if ! echo "$CMD" | grep -qiE '(api\.anthropic\.com|github\.com|registry\.npmjs\.org|localhost|127\.0\.0\.1|100\.(6[4-9]|[7-9][0-9]|1[0-2][0-7])\.|stephenfeather\.com|100\.98\.16\.42|api\.newrelic\.com)'; then
