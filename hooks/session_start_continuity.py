...
def ensure_memory_daemon() -> str | None:
    ...
    try:
        ...
        possible_locations = [
            # 1. CLAUDE_OPC_DIR environment variable (canonical)
            *([] if not opc_dir else [Path(opc_dir) / "scripts" / "core" / "memory_daemon.py"]),
            # 2. Relative to hook (development setup)
            Path(__file__).parent.parent.parent / "opc" / "scripts" / "core" / "memory_daemon.py",
            # 3. In .claude/scripts/core/ (wizard-installed)
            Path(__file__).parent.parent / "scripts" / "core" / "memory_daemon.py",
            # 4. Global ~/.claude/scripts/core/ (global install)
            Path.home() / ".claude" / "scripts" / "core" / "memory_daemon.py",
            # 5. Legacy ~/.opc-dev location
