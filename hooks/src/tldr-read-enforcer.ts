...
`;
  ...
  // Allow the read but cap it, inject TLDR context
  const DEFAULT_READ_LIMIT = 100;

  const modelContext = `TLDR context for ${basename(filePath)} (${layerNames}):
${tldrContext}${crossFileSection}
Read was capped at ${DEFAULT_READ_LIMIT} lines. Use offset/limit for more.`;
