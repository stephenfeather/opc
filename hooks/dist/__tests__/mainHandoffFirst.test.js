...
`;
            ...
            assert.ok(fullOutput.includes('Handoff goal') || fullOutput.includes('Working from handoff'), 'Should include content from handoff Ledger, not legacy ledger');
            assert.ok(!fullOutput.includes('Legacy ledger goal') && !fullOutput.includes('Legacy focus'), 'Should NOT include content from legacy ledger');
        });
...
`;
            ...
            const fullOutput = JSON.stringify(output);
            assert.ok(fullOutput.includes('New session goal') || fullOutput.includes('New session focus'), 'Should include content from most recent handoff');
        });
    });
...
`;
            ...
            const fullOutput = JSON.stringify(output);
            assert.ok(fullOutput.includes('Fallback legacy goal') || fullOutput.includes('Legacy fallback focus'), 'Should fall back to legacy ledger when handoff has no Ledger section');
        });
    });
...
`;
            ...
            const fullOutput = JSON.stringify(output);
            assert.ok(fullOutput.includes('Pure legacy goal') || fullOutput.includes('Pure legacy focus'), 'Should use legacy ledger when no handoffs exist');
        });
...
`;
            ...
            const fullOutput = JSON.stringify(output);
            assert.ok(fullOutput.includes('Test goal') || fullOutput.includes('Test focus'), 'Should fall back to legacy ledger');
        });
...
`;
            ...
            const fullOutput = JSON.stringify(output);
            assert.ok(fullOutput.includes('Resume test goal') || fullOutput.includes('Working on resume'), 'Resume should include handoff Ledger content');
        });
    });
});
