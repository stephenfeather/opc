...
async function main() {
  ...
  // LITERAL: Allow Grep, inject TLDR hint as additionalContext
  if (queryType === 'literal') {
    trackHookActivitySync('smart-search-router', projectDir, true, {
      queries_routed: 1, literal_queries: 1,
    });

    const context = `Search routing: literal query "${pattern}". For richer context (call graph, docstrings), also try: tldr search "${pattern}" ${projectDir} — or Read the target file (tldr-read-enforcer will enrich).`;
  ...
  // STRUCTURAL: Allow Grep, inject AST-grep hint as additionalContext
  if (queryType === 'structural') {
    ...
    const context = `Search routing: structural query "${pattern}". For precise AST matching, consider: mcp__ast-grep__find_code with pattern "${astPattern}". For richer context (call graph, docstrings): tldr search "${target || pattern}" ${projectDir}`;
  ...
  // SEMANTIC: Run semantic search, inject results as additionalContext, allow Grep as fallback
  trackHookActivitySync('smart-search-router', projectDir, true, {
  ...
  if (semanticSearch.status === 'ok' && semanticSearch.results.length > 0) {
    ...
    context = `Search routing: semantic query "${pattern}". TLDR semantic results: ${resultsStr}. Read the most relevant file for deeper analysis.`;
  } else if (semanticSearch.status === 'no_index') {
    context = `Search routing: semantic query "${pattern}". No semantic index available. Run "tldr semantic index . --lang all" to enable. Grep results below are the fallback.`;
  } else if (semanticSearch.status === 'daemon_unavailable') {
    context = `Search routing: semantic query "${pattern}". TLDR daemon not running. Grep results below are the fallback.`;
  } else if (semanticSearch.status === 'indexing') {
    context = `Search routing: semantic query "${pattern}". TLDR daemon is building the semantic index. Grep results below are the fallback.`;
  } else {
    context = `Search routing: semantic query "${pattern}". No semantic matches found. Grep results below are the fallback.`;
  }
