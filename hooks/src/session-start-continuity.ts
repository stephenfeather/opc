...
/**
 * Ensure the global memory extraction daemon is running.
 * The daemon monitors for stale sessions and auto-extracts learnings.
 * Returns a status string or null if already running.
 */
function ensureMemoryDaemon(): string | null {
  const pidFile = path.join(process.env.HOME || '', '.claude', 'memory-daemon.pid');
  ...
  if (fs.existsSync(pidFile)) {
    try {
      ...
      // Stale PID file or process gone — remove and restart
      try { fs.unlinkSync(pidFile); } catch { /* ignore */ }
    }
  }

  // Find memory_daemon.py via OPC path resolution
  const opcDir = getOpcDir();
  const candidates = [
    opcDir ? path.join(opcDir, 'scripts', 'core', 'memory_daemon.py') : null,
    path.join(process.env.HOME || '', '.claude', 'scripts', 'core', 'memory_daemon.py'),
  ].filter((p): p is string => p !== null && fs.existsSync(p));

  if (candidates.length === 0) {
    console.error('Memory daemon script not found');
    return null;
  }

  const daemonScript = candidates[0];
  ...
  try {
    ...
    console.error(`Failed to start memory daemon: ${e}`);
    return null;
  }
}
