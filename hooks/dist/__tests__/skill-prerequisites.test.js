...
import { resolvePrerequisites, detectCircularDependency, } from '../skill-router.js';
describe('Prerequisite Resolution', () => {
    ...
    it('should throw CircularDependencyError for A -> B -> A cycle', () => {
        ...
        expect(() => resolvePrerequisites('skill-a', rules)).toThrow(CircularDependencyError);
    });
    ...
    it('should handle multiple roots in dependency graph', () => {
        ...
        expect(result.loadOrder.indexOf('combined-skill')).toBeGreaterThan(result.loadOrder.indexOf('dep-a'));
        expect(result.loadOrder.indexOf('combined-skill')).toBeGreaterThan(result.loadOrder.indexOf('dep-b'));
    });
});
